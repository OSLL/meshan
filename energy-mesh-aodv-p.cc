...
outfile1.open("flows_drain_speed.txt", ios::app);
...
if(remainingEnergy<0.049000)
NS_LOG_UNCOND("BATTERY IS LOW....,Change path");
//outfile3<<"battery is low..... change path";
// Energy draining speed calculation



outfile1<<"at time "<<t<<"s\n";
t++;
for(int k=1;k<10;k++)
{
//outfile1<<at 
speed=(oldValue-remainingEnergy)/(Simulator::Now ().GetSeconds ()-(Simulator::Now ().GetSeconds ()-1));
//NS_LOG_UNCOND("Speed at which battery is draining  "<<speed<<"%/sec");
//if(Simulator::Now().GetSeconds()==10 ||Simulator::Now().GetSeconds()==20||Simulator::Now().GetSeconds()==30||Simulator::Now().GetSeconds()==40||Simulator::Now().GetSeconds()==50)
...
basicSourceHelper.Install(nodes);
WifiRadioEnergyModelHelper radioEnergyHelper;
radioEnergyHelper.Set("TxCurrentA", DoubleValue(0.0174));
DeviceEnergyModelContainer deviceModels =
radioEnergyHelper.Install(wifiDevices, sources);
//energy source is connected to node 2    
//std::cout<<"node"<<m_dest;
Ptr<BasicEnergySource> basicSourcePtr = DynamicCast<BasicEnergySource>
(sources.Get(m_source));
// energy source is connected to node 3
